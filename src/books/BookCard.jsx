...
export default function BookCard({ book }) {
  const { book_image, book_name, author_name, publication_year, rating, price } = book;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-center rounded-md border border-[#324251]/30 bg-white p-4">
        <img className="max-w-[144px]" src={book_image} alt="book name" />
      </div>
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-1">
            {rating.map((item) => (
              <img key={item.id} src={item.star} />
            ))}
            <span className="text-xs lg:text-sm">({rating.length} Star)</span>
          </div>
        </div>
